...
async def add_partner(message: Message):
    ...
    if len(parts) != 3:
        # Исправлено: убираем угловые скобки или отключаем parse_mode
        await message.answer("Использование: /add_partner telegram_id имя", parse_mode=None)
        return
    _, tele_id_str, name = parts
    try:
        tele_id = int(tele_id_str)
    except:
        await message.answer("Ошибка: telegram_id должно быть числом.", parse_mode=None)
        return
    async with async_session_maker() as session:
        existing = await session.execute(select(Partner).where(Partner.telegram_id == tele_id))
        if existing.scalar_one_or_none():
            await message.answer("Партнёр с таким Telegram ID уже существует.", parse_mode=None)
            return
        partner = Partner(telegram_id=tele_id, name=name, referral_link=f"partner_{tele_id}")
        session.add(partner)
        await session.commit()
    await message.answer(f"✅ Партнёр {name} добавлен. Его ссылка: partner_{tele_id}", parse_mode=None)
...
async def list_partners(message: Message):
    ...
    async with async_session_maker() as session:
        ...
        if not partners:
            await message.answer("Нет партнёров.", parse_mode=None)
            return
...
async def list_manual_topups(message: Message):
    ...
    if not topups:
        await message.answer("Нет ручных пополнений на проверке.", parse_mode=None)
        return
    for t in topups:
...
async def confirm_manual_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("✅ Пополнение подтверждено, баланс пользователя увеличен.", parse_mode=None)
...
async def list_withdraw_requests(message: Message):
    ...
    if not reqs:
        await message.answer("Нет заявок на вывод.", parse_mode=None)
        return
    for r in reqs:
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def approve_withdraw_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text("✅ Заявка на вывод одобрена.", parse_mode=None)
    await callback.answer()
